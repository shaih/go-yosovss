...
func StartCommitteeParty(
	...
	for rounds := 0; rounds < totalRounds; rounds++ {
		...
		nextHoldCommittee := common.NextCommitteeDeterministic(holdCommittee, len(pks))
		nextVerCommittee := common.NextCommitteeDeterministic(verCommittee, len(pks))
	...
	m, err := oldvss.VSSReconstruct(params, shares, verifications)
	if err != nil {
...
func HoldingCommitteeShareProtocol(
	...
	bi, vi, di, wi, ei, err := common.TwoLevelShare(params, share.S, share.R, t, n)
	if err != nil {
	...
	biEnc, err := common.EncryptSharesForVer(pks, bi, verCommittee)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("error in encrypting s_i shares: %v", holdIndex)
	}

	// Encrypt D_i for verification committee
	diEnc, err := common.EncryptSharesForVer(pks, di, verCommittee)
	if err != nil {
	...
	biResponse := make([][]*oldvss.Share, n)
	diResponse := make([][]*oldvss.Share, n)
	for j := 0; j < n; j++ {
		for k := 0; k < n; k++ {
			biResponse[j] = make([]*oldvss.Share, n)
			diResponse[j] = make([]*oldvss.Share, n)
		}
	}
...
func VerificationCommitteeProtocol(
	...
	bk := make([][]*oldvss.Share, n)
	dk := make([][]*oldvss.Share, n)

	for i := 0; i < n; i++ {
		bk[i] = make([]*oldvss.Share, n)
		dk[i] = make([]*oldvss.Share, n)
	}
	...
	for i, holder := range holdCommittee {
		...
		if !bikFailed {
			...
			for j := 0; j < n; j++ {
				bIsValid, _ := oldvss.VSSVerify(params, bik[j], holdShareMsg.Vi[j])
				if bIsValid {
					bk[i][j] = &bik[j]
				} else {
					bk[i][j] = nil
		...
		if !dikFailed {
			...
			for j := 0; j < n; j++ {
				dIsValid, _ := oldvss.VSSVerify(params, dik[j], holdShareMsg.Wi[j])
				if dIsValid {
					dk[i][j] = &dik[j]
				} else {
					dk[i][j] = nil
...
func HoldingCommitteeReceiveProtocol(
	...
	bj := make([][]oldvss.Share, n)
	dj := make([][]oldvss.Share, n)
	for i := 0; i < n; i++ {
		bj[i] = make([]oldvss.Share, n)
		dj[i] = make([]oldvss.Share, n)
	}
	...
	for i < n && len(aj) < t {
		...
		aij, err1 := oldvss.VSSReconstruct(params, bij, v[i][holdIndex])
		cij, err2 := oldvss.VSSReconstruct(params, dij, w[i][holdIndex])
	...
	for i := 0; i < t; i++ {
		...
		isVerified, err := oldvss.VSSVerify(params, testShare, e[indices[i]-1])
		if err != nil {
