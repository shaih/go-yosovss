...
// Test both scalar and point scalar multiplication with xy points
func TestMatrixMulXY(t *testing.T) {
	g := RandomPointXY()
	...
	mat1Point, err := PointXYScalarMatrixMul(g, mat1)
	if err != nil {
		t.Error(err)
	}
	...
	expectedMulPoint, err := PointXYScalarMatrixMul(g, expectedMul)
	if err != nil {
		t.Error(err)
	}
	...
	actualMulPoint, err := PointXYMatrixScalarMatrixMul(mat1Point, mat2)
	if err != nil {
		t.Error(err)
	}

	if !PointXYMatrixEqual(expectedMulPoint, actualMulPoint) {
		t.Errorf("incorrect point x scalar multiplication result")
	}
}
